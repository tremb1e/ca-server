...
async def test_auth_manager_secondary_vote_emits_only_after_ten_primary_results(tmp_path, monkeypatch):
    ...
    # Secondary now reports on the sigmoid(policy) axis. Raw scores [-1.0]*8 + [1.0]*2
    # under ema primary (alpha=1.0, policy.threshold=0.0) -> primary scores
    # sigmoid(-1.0) (reject) and sigmoid(1.0) (accept); primary threshold = sigmoid(0.0).
    primary_threshold = 1.0 / (1.0 + math.exp(0.0))
    boundary_score = 1.0 / (1.0 + math.exp(1.0))  # sigmoid(-1.0), the 8th smallest score
    assert result.score == pytest.approx(boundary_score)
    assert result.threshold == pytest.approx(primary_threshold)
    # Verdict <-> reported-score self-consistency on the shared policy axis.
    assert (result.score >= result.threshold) == result.accept
    # Not the old reject-ratio scale (8-of-10 would have reported 0.2 vs 0.3).
...
async def test_auth_manager_secondary_consumes_multiple_primary_intervals_from_one_packet(tmp_path, monkeypatch):
    ...
    # Secondary score/threshold share the sigmoid(policy) axis and stay self-consistent.
    primary_threshold = 1.0 / (1.0 + math.exp(0.0))
    assert result.threshold == pytest.approx(primary_threshold)
    assert result.score == pytest.approx(1.0 / (1.0 + math.exp(1.0)))  # sigmoid(-1.0)
    assert (result.score >= result.threshold) == result.accept
...
@pytest.mark.asyncio
async def test_result_rows_split_primary_and_secondary_axes(tmp_path, monkeypatch):
    """Contract F: primary rows carry raw_/display_ fields, secondary rows carry
    secondary_/display_ fields, and each row stamps result_stage."""
    import json
    ...
    for _ in range(10):
        await manager.handle_packet(user_id=user_id, session_id=session_id, parsed_batch={})

    results_path = tmp_path / "inference" / user_id / session_id / "results.jsonl"
    rows = [json.loads(line) for line in results_path.read_text(encoding="utf-8").splitlines()]

    primary_rows = [r for r in rows if r["result_stage"] == "primary"]
    secondary_rows = [r for r in rows if r["result_stage"] == "secondary"]
    assert len(primary_rows) == 10
    assert len(secondary_rows) == 1

    for row in primary_rows:
        assert row["result_stage"] == "primary"
        # Explicit primary multi-axis fields are present.
        for key in ("raw_score", "raw_threshold", "display_score", "display_threshold"):
            assert key in row, f"primary row missing {key}"
        # raw axis is the clean -MSE score; display axis is the [0,1] decision score.
        assert row["raw_threshold"] == pytest.approx(0.0)
        assert 0.0 <= float(row["display_score"]) <= 1.0
        assert 0.0 <= float(row["display_threshold"]) <= 1.0

    secondary = secondary_rows[0]
    assert secondary["result_stage"] == "secondary"
    for key in ("secondary_score", "secondary_threshold", "display_score", "display_threshold"):
        assert key in secondary, f"secondary row missing {key}"
    # Secondary display axis mirrors the secondary score/threshold.
    assert secondary["display_score"] == pytest.approx(secondary["secondary_score"])
    assert secondary["display_threshold"] == pytest.approx(secondary["secondary_threshold"])
