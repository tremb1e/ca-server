...
def test_load_best_policy_reads_ema_strategy(tmp_path) -> None:
    user = "user_a"
    models_root = tmp_path / "data_storage" / "models"
    user_dir = models_root / user
    ckpt = user_dir / "checkpoints" / "model.pt"
    ...
    assert loaded.interrupt_rule == "ema"
    assert loaded.decision_strategy == "ema"
    assert loaded.ema_alpha == 0.35
