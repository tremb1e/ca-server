...
def _normalise_auth_result_row(row: Dict[str, Any], *, device_id: str, session_id: str) -> Dict[str, Any]:
    payload = dict(row)
    payload.setdefault("device_id_hash", str(device_id))
    payload.setdefault("session_id", str(session_id))
    if "score" not in payload and "decision_score" in payload:
        payload["score"] = payload.get("decision_score")
    if "threshold" not in payload and "decision_threshold" in payload:
        payload["threshold"] = payload.get("decision_threshold")
    if "accept" not in payload and "decision_accept" in payload:
        payload["accept"] = payload.get("decision_accept")
    if "normalized_score" not in payload and "decision_score" in payload:
        payload["normalized_score"] = payload.get("decision_score")
    payload.setdefault("interrupt", False)
    payload.setdefault("k_rejects", 0)
    payload.setdefault("vote_recent_windows", 0)
