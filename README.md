...
如果设备 ID 以 `-` 开头，server CLI 会自动把 `--user <device_id_hash>` 归一化为等号形式传给子命令；直接调用 `ca_train/hmog_vqgan_experiment.py` 时请使用 `--users=<device_id_hash>`。
...
- 在线认证默认启用二次“迟滞决策”：正常 1 秒上传包会贡献 1 个一次迟滞结果；若单包积压多秒数据，server 会按 `primary_result_interval_sec` 从一次窗口序列中抽样多个结果进入二次迟滞。二次默认使用 `8-of-10` 投票，正常上传节奏下按 `10 * 1s` 约每 10 秒向 App 返回一个最终 `AuthResult`。ACK 仍按每个 `DataPacket` 即时返回。
