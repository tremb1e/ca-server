...
from utils_dataset import train_test_split
...
class DatasetLoader:
    ...
    def split_train_valid_train_test(
        ...
        subjects = [s for s in subjects if s not in subjects_v_train]
        '''
        subjects_v_test = [  # NOQA
            s for s in subjects if s not in subjects_v_train
        ]
        '''
        subjects_v_test = random.sample(subjects, n_valid_test)
    ...
    def split_train_valid_train_test_distribute(
        self,
        n_valid_train,
        n_valid_test,
        n_test_train,
    ...
    def split_train_valid_train_test_transfer(
        self,
        dframe,
        n_valid_train,
        n_valid_test,
    ...
    def split_train_valid_train_test_transfer_distribute(
        self,
        dframe,
        n_valid_train,
        n_valid_train_select_list,
        n_valid_test,
        n_valid_test_select_list,
        n_test_train,
        n_test_train_select_list,
        n_test_test,
        n_test_test_select_list,
        cleanup=True,
    ):
        """Create three splits of the datasets by splitting by subjects.
        ...
        #random.seed(self.seed)
        subjects_t_test = random.sample(n_test_test_select_list, n_test_test)
        print("the selected test_test is:", subjects_t_test)
        
        #random.seed(self.seed)
        subjects_t_train = random.sample(n_test_train_select_list, n_test_train)
        print("the selected test_train is:", subjects_t_train)
        
        #random.seed(self.seed)
        subjects_v_train = random.sample(n_valid_train_select_list, n_valid_train)
        print("the selected valid_train is:", subjects_v_train)
        
        #random.seed(self.seed)
        subjects_v_test = random.sample(n_valid_test_select_list, n_valid_test)
        print("the selected valid_test is:", subjects_v_test)
    ...
    def generate_train_valid_train_test_data_back(
            self,
            dframe,
            n_valid_train,
            n_valid_test,
            ...
            subjects = df["subject"].unique().tolist()
            #random.seed(self.seed)
            subjects_v_train = random.sample(subjects, 50)
            subject_list = []
            for subject, df_group in tqdm(df.groupby("subject"), desc="Session", leave=False):
                if len(df_group["subject"]) > 450:
                    subject_list.append(subject)
                
            #print("the length of subjects_v_train is:", len(subjects_v_train))
            '''
            df = df.query("subject in @subjects_v_train").copy()
            data_trian ,data_test = train_test_split(df, test_size=0.4)
            self.valid_train, self.valid_test = train_test_split(data_trian, test_size=0.4)
            self.test_train, self.test_test = train_test_split(data_test, test_size=0.2)
            '''
            df = df.query("subject in @subject_list").copy()
            data_trian ,data_test = train_test_split(df, test_size=0.95)
            self.valid_train, self.valid_test = train_test_split(data_trian, test_size=0.4)
            self.test_train, self.test_test = train_test_split(data_test, test_size=0.1)
            
            if cleanup:
                del self.all
                self.all = None
    
    def generate_train_valid_train_test_data(
            self,
            dframe,
            n_valid_train,
            n_valid_test,
            ...
            #random.seed(self.seed)
            subjects_v_train = random.sample(subjects, len(subjects))
            #print("the length of subjects_v_train is:", len(subjects_v_train))
            '''
            df = df.query("subject in @subjects_v_train").copy()
            data_trian ,data_test = train_test_split(df, test_size=0.4)
