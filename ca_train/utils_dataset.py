...
def utils_plot_detect_delay(df_plot, factor, xlim):
    ...
    for owner, df_group in df_plot[["owner", "test_eer"]].groupby("owner"):
        ...
        df_temp = df_temp.sample(frac=1, random_state=712).reset_index(drop=True)
...
def utils_custom_scale(
    ...
    if scope in ["session", "subject"]:
        df_subject = []
        for subject_item in tqdm(subjects, desc=f"normalize"):
            df_subject.clear()
            df_subject.append(subject_item)
            df_temp = df.query("subject in @df_subject").copy()
            scaler = utils_get_scaler(scaler_name)
            #print("the group name is:",subject_item)
            #print("the df_group head is:", df_temp.head(5))
            if len(scale_cols) > 1:
                ...
                if scaler_name == "minmax_fix":
                    ...
                    #print("the data is:", df_temp[scale_cols].values[np.isinf(df_temp[scale_cols].values)])
    ...
    if scope in ["session", "subject"]:
        for group_name, df_group in tqdm(
            ...
            print("the df_group head is:", df_group.head(5))
            if len(scale_cols) > 1:
                # Features come in different columns
                if scaler_name == "minmax_fix":
                    scaler.fit(get_dummy_data(feature_cols))
...
def utils_custom_scale_all(
    ...
    if scope in ["session", "subject"]:
        ...
        for subject_item in tqdm(subjects, desc=f"normalize"):
            ...
            df_subject.append(subject_item)
            df_temp = df.query("subject in @df_subject").copy()
            num = math.ceil(len(df_temp) * 0.7)
            df_temp = df_temp.iloc[:num]
            scaler = utils_get_scaler(scaler_name)
            #print("the group name is:",subject_item)
            #print("the df_group head is:", df_temp.head(5))
            if len(scale_cols) > 1:
...
def utils_reshape_features(df, feature_cols, window_size, step_width):
    ...
    for session, df_group in tqdm(df.groupby("session"), desc="Session", leave=False):
        ...
        task_type = df_group["task_type"].mode()
        # Extract features
        features = df_group[feature_cols].values
        #print("the len is:", len(features), session)
...
#this function is noted by humingming
...
def utils_generate_cv_scenarios_single(
    df,
    seed, 
    hmog,   
    anchor_subject_name,
    ...
    df_train, df_valid_original = hmog.split_sessions(
        df, sessions_per_type=1, seed=50
    )
    '''
    df_train, df_valid_original = train_test_split(df, test_size=0.4)
    anchor_index = np.where(df["subject"] == anchor_subject_name)[0]
    df_anchor_train, df_anchor_original = train_test_split(df.iloc[anchor_index], test_size=0.4)
    '''
    print("the shape of df_train is:", df_train.shape)
    print("the shape of df_valid_original is:", df_valid_original.shape)
    print("the subjects list is:\n", list(df_train["subject"].unique()))
    print("the subjects list is:\n", list(df_valid_original["subject"].unique()))
    '''
    ...
    '''
    print("the n_sessions is:", n_sessions)
    print("the samples_per_session_train is:", samples_per_session_train)
    print("the samples_per_session_test is:", samples_per_session_test)
    '''
    # PREPARE TRAINING DATA
    # ============================
    #samples_per_session_train = samples_per_subject_train
    #samples_per_session_test = samples_per_subject_test
    ...
    df_owner_train = (
        ...
        .apply(
            lambda x: x.sample(
                #n=min(len(x), samples_per_session_train)
                n=len(x), random_state=seed
            ).copy()
        )
    )
    df_owner_train["label"] = 1
    df_owner_train["mask"] = -2  # To use as sklearn cv mask
    #print("the len of df_owner_train", len(df_owner_train))
    ...
    if limit_train_samples:
        df_owner_train = df_owner_train.sample(
            #n=min(len(df_owner_train), limit_train_samples)
            n=len(df_owner_train), random_state=seed
    ...
    df_owner_valid = (
        ...
        .apply(
            ...
            lambda x: x.sample(n=len(x), random_state=seed)
        )
        .copy()
    )
    ...
    # --------------------------------
    '''
    print("the len of df_owner_valid", len(df_owner_valid))
    print("the impostors is:\n", impostors)
    '''
    ...
    for idx, impostor in enumerate(impostors):
        ...
        df_impostor = (
            ...
            .apply(
                lambda x: x.sample(
                    #n=min(len(x), samples_per_session_test)
...
def utils_generate_cv_scenarios(
    ...
    '''
    df_train, df_valid_original = train_test_split(df, test_size=0.2)
    '''
    print("the shape of df_train is:", df_train.shape)
    print("the shape of df_valid_original is:", df_valid_original.shape)
    print("the subjects list is:\n", list(df_train["subject"].unique()))
    ...
    for owner in subjects:
        ...
        # Shuffle samples
        df_cv_scenarios = df_cv_scenarios.sample(frac=1)

        # Pass over data along with owner & impostor info
        yield (df_cv_scenarios, owner, impostors)


def utils_generate_cv_scenarios_tenfold(
    df,
    seed, 
    hmog,   
    samples_per_subject_train,
    ...
    for owner in subjects:
        ...
        user_data_len = len(df_user_train)
        df_owner_train, df_owner_valid = train_test_split(df_user_train, test_size=0.1)
        ...
        if limit_test_samples:
            df_owner_valid = df_owner_valid.sample(
                n=min(len(df_owner_valid), limit_test_samples)
            )
        ...
        for idx, impostor in enumerate(impostors):
            # Select impostor validation set
            df_impostor = (
                df.query("subject == @impostor")
                .groupby("session", group_keys=False)
                .apply(
                    lambda x: x.sample(
                        #n=min(len(x), samples_per_session_test), random_state=seed
                        n=min(len(x), int(user_data_len/(len(subjects)-1)))
                    ).copy()
                )
            )
            df_impostor["label"] = -1
...
def utils_generate_cv_scenarios_tenfold_(
    df,
    seed, 
    hmog,   
    samples_per_subject_train,
...
def utils_generate_cv_scenarios_tenfold_transformer(df):
    """Generate pairs of owner training data with owner/impostor validation data, along with CV mask."""
    # Split into training and validation set
    '''
    df_train, df_valid_original = hmog.split_sessions(
...
def utils_generate_cv_scenarios_tenfold_t(
    df,
    seed, 
    hmog,   
    samples_per_subject_train,
    ...
    for owner in subjects:
        ...
        user_data_len = len(df_user_train)
        df_owner_train, df_owner_valid = train_test_split(df_user_train, test_size=0.2)
...
def user_aware_split(data, test_size=0.3):
    users = data['subject'].unique()
    n_test_users = int(len(users) * test_size)
    test_users = np.random.choice(users, n_test_users, replace=False)
    train_data = data[~data['subject'].isin(test_users)]
    test_data = data[data['subject'].isin(test_users)]
