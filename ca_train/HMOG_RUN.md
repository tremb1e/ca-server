...
本文档描述当前服务端代码的训练、策略搜索和离线认证流程。旧版 HMOG 实验文档中“五个固定 HMOG 用户、12 轴输入、CUDA-only”的说明已经过期；当前主链路以服务端生成的 window CSV 为输入，VQGAN 根据每用户 `sensor_mode.json` 使用 6 或 9 轴。
...
2026-07-13 起按用户磁力计质量选择 `input_height=6/9`（`image_channels` 仍为 1）：模长超过 `120µT` 的比例大于 `1%`、覆盖率低于 `95%` 或有效样本少于 `100` 时使用 6 轴，否则使用 9 轴。轴数写入 `processed_data/z-score/<user>/sensor_mode.json` 并传递给训练与推理；仍不拼接额外 magnitude 通道。轴数与本次判定不一致的旧 checkpoint 不可复用。
...
- checkpoint 和 config：包含与 `sensor_mode.json` 一致的 `input_height = 6` 或 `9`。
